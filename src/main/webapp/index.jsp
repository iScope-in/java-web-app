<html>
	<head>
		<title>iScope</title>
	</head>
	<body>
		<h2 style="background: blue;text-align: center;color: white;">iScope's DevOps Projects</h2>
		<p><b>iScope's DevOps</b></p>
		<p>We need to have the knowledge of Linux installation, Git, maven, web application, aws to complete the project </p>
		<p>A DevOps Engineer must have knowledge on the below tools</p>
		<ol>
		<li>Linux Administration</li>
		<li>Linux Networking Commands</li>
		<li>Linux Shell Scripting</li>
		<li>AWS Cloud Services specific for DevOps Engineers</li>
		<li>Azure Cloud Services specific for DevOps Engineers</li>
		<li>Git & GitHub / GitLab / BitBucket</li>
		<li>Maven / SonarQube / Jfrog</li>
		<li>Ansible</li>
		<li>Docker</li>
		<li>Jenkins</li>
		<li>Kubernetes</li>
		</ol>
		<p>You are most welcome to iScope's DevOps master program</p>
	</body>
</html>
